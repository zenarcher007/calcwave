...
class WindowManager:
  def __init__(self, global_config, scr, initialExpr, audioClass, exportDtype = int):
    ...
    self.menu = UIManager(Box(rowSize = 2, colSize = cols, rowStart = rows - 7, colStart = 0), global_config, audioClass, self.infoDisplay, self.editor, exportDtype = exportDtype)
  ...
  def windowThread(self, global_config, scr, menu, audioClass):
    ...
    try:
      while self.global_config.shutdown is False and self.shutdown is False:
        ch = self.scr.getch()
        ...
        if successful and self.focused == self.editor:
          ...
          with global_display_lock:
            self.editor.win.refresh()
          continue
        
        # Switch between menu and inputPad with the arrow keys
        if self.menu.isEditing():
          with global_display_lock:
            self.focused.refresh()
          continue # Don't remove focus from the menu while it's editing
        ...
        with global_display_lock:
          self.focused.refresh() # A problem: refresh on some objects will redraw their text; this may cause highlighting to disappear
          # Challenge: call onHoverEnter, but do not call refresh.
...
class UIManager:
  def __init__(self, shape: Box, global_config, audioClass, infoDisplay, editor, exportDtype = int):
    ...
    self.menuItems.append(graphBtn)
    self.menuItems.append(startWin)
    self.menuItems.append(progressWin)
    self.menuItems.append(endWin)
    self.menuItems.append(saveWin)
    self.menuItems.append(stepWin)
    ...
    if self.editing == False and (ch == curses.KEY_LEFT or ch == curses.KEY_RIGHT):
      ...
      if ch == curses.KEY_LEFT:
        if self.focusedWindowIndex > 0:
          ...
          self.focusedWindowIndex = len(self.menuItems) - 1
      elif ch == curses.KEY_RIGHT:
        if self.focusedWindowIndex < len(self.menuItems) - 1:
          self.focusedWindowIndex = self.focusedWindowIndex + 1
        else:
          self.focusedWindowIndex = 0
      self.focusedWindow = self.menuItems[self.focusedWindowIndex]
